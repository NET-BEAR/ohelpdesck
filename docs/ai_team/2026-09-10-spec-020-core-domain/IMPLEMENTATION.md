...
Статус: `rework_in_progress`. Дата: 2026-09-10.
...
Channel, затем canonical `(channel_id, external_message_id)` через transaction
advisory lock и возвращает исходные Contact/Identity/Conversation/Message IDs
до создания любых facts. Поэтому повтор внешней доставки с другими sender или
thread не создаёт пустой Conversation. Только для нового external message
сервис блокирует отсутствующие Identity/Conversation через advisory locks,
...
Повторная проверка P1 добавляет два negative/concurrent сценария: existing
duplicate с изменёнными sender/thread и параллельные duplicate deliveries с
разными sender/thread. Оба требуют вернуть один canonical result и оставить
ровно один ContactIdentity, Conversation, Message и исходные два outbox event.

Cross-table equality проверяется явно в service code: выбранная Identity
должна принадлежать текущему Channel; Conversation должна ссылаться на ту же
Identity, Contact и Channel; canonical Message path также проверяется до
возврата результата. Скрытые PostgreSQL triggers/functions будут удалены
отдельной новой checksummed migration `000004_remove_core_channel_triggers`, потому что revision `000003` уже
применён на dev-контуре и изменять его source нельзя.
