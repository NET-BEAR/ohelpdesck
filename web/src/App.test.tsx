...
it('shows pending then readiness and dependency states', async () => {
  vi.spyOn(api, 'getReadiness').mockResolvedValue({ status: 'ready', checks: { postgres: 'ok', redis: 'degraded' } });
  mount();
...
it('shows a safe error and supports explicit retry', async () => {
  const request = vi.spyOn(api, 'getReadiness').mockRejectedValueOnce(new Error('API недоступен (HTTP 503)')).mockResolvedValue({ status: 'ready', checks: { postgres: 'ok' } });
  mount('/health');
