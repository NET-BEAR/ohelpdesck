...
function Timeline({ value, busy, quiet, onBefore, onAfter }: { value: unknown; busy: boolean; quiet: boolean; onBefore: (cursor: string) => void; onAfter: (cursor: string) => void }) {
  const title = useRef<HTMLHeadingElement>(null); const wasBusy = useRef(false);
  useEffect(() => { if (wasBusy.current && !busy) title.current?.focus(); wasBusy.current = busy; }, [busy]);
...
function WorkspacePage() {
  ...
  return <section aria-labelledby="workspace-title"><h1 id="workspace-title">Обращения</h1><fieldset aria-label="Фильтры очереди"><label>Статус<select aria-label="Фильтр статуса" value={status} onChange={e => resetFilter('status', e.target.value)}><option value="">Все</option><option value="open">Открытые</option><option value="pending">В ожидании</option><option value="resolved">Решённые</option></select></label><label>Приоритет<select aria-label="Фильтр приоритета" value={priority} onChange={e => resetFilter('priority', e.target.value)}><option value="">Все</option><option value="high">Высокий</option><option value="urgent">Срочный</option></select></label><label>Назначение<select aria-label="Фильтр назначения" value={assignee} onChange={e => resetFilter('assignee', e.target.value)}><option value="">Все</option><option value="me">На мне</option><option value="unassigned">Без назначения</option></select></label></fieldset><button onClick={() => void queue.refetch()} disabled={queue.isFetching}>Обновить</button>{queue.data.items.length === 0 ? <p aria-live="polite">В доступных каналах обращений нет.</p> : <ul>{queue.data.items.map(item => <li key={item.id}><Link to={`/workspace/${item.id}`}>#{item.number} · {item.contact.display_name}</Link><p>{item.channel.name} · {item.status} · {item.priority}</p></li>)}</ul>}<nav aria-label="Страницы очереди"><button aria-label="Предыдущая страница" disabled={history.length === 0} onClick={() => { const previous = history.at(-1) ?? ''; const next = new URLSearchParams(search); next.delete('previous'); history.slice(0, -1).forEach(value => next.append('previous', value)); if (previous) next.set('cursor', previous); else next.delete('cursor'); setSearch(next); }}>Назад</button><button aria-label="Следующая страница" disabled={!queue.data.next_cursor} onClick={() => { const next = new URLSearchParams(search); next.append('previous', cursor); next.set('cursor', queue.data.next_cursor ?? ''); setSearch(next); }}>Далее</button></nav></section>;
}
function ConversationPage() {
  ...
  return <section><p><Link to="/workspace">К обращениям</Link></p><h1>Диалог</h1>
    ...
    <pre aria-label="Состояние диалога">{JSON.stringify(conversation, null, 2)}</pre><Timeline value={messages.data} busy={messages.isFetching} quiet={replyText !== ''} onBefore={before => setTimelineCursor({ before })} onAfter={after => setTimelineCursor({ after })} /></section>;
}
